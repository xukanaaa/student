...
space_size = 7000; % 空间大小（长、宽、高均为1km）
...
for i=1:num_drones
    Neb_list{i}=zeros(num_drones*2,broad*2+2+1+num_drones);
end
...
for j=1:simulation_k
    for i=1:num_drones
        [~,d]=sort(t_global_total(:,j));
             ...
             %第一轮不更新，从第二轮开始
        if j>=2
            M=MulBroVirLink_alphaTwoRouTnoise_l(Neb_list{d(i)},j,num_drones,l(d(i),1));%更新逻辑时钟频偏参数
