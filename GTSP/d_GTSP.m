        ...
        for i=1:num_drones
            ...
            for k=1:num_drones
                if A(d(i),k)==1
                    ...
                    C(d(i),7)=C(d(i),3);
                    C(d(i),1)=j;
                    C(d(i),2)=t_local_total(d(i),j)+8e-7*randn;
                    C(d(i),3)=(((t_local_total(d(i),j)-beta(d(i),1))/(alpha(d(i),1)))+B(d(i),k)/3e8)*alpha(k,1)+beta(k,1)+8e-7*randn;
...
format long;
plot(10:simulation_k,skewFinal(10:end),'-*','LineWidth',1);
xlabel('同步轮次');
ylabel('最大频率偏差');
