...
for j=1:simulation_k
    for i=1:num_drones
        [c,d]=sort(t_global_total(:,j));
             ...
             %第一轮不更新，从第二轮开始
        if j>=2
            l(d(i),1)=xukan_update_l(Neb_list{d(i)},j,num_drones,l(d(i),1));%更新逻辑时钟频偏参数
        ...
        for k=1:num_drones
            if A(d(i),k)==1
                C=Neb_list{k};
                C(d(i),1)=j;
                C(d(i),2)=t_local_total(d(i),j);%+2e-6*randn+exprnd(4e-6);
                C(d(i),3)=(((t_local_total(d(i),j)-beta(d(i),1))/(alpha(d(i),1)))+B1(d(i),k)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),4)=t_local_total(d(i),j)+dt;%+2e-6*randn+exprnd(4e-6);
                C(d(i),5)=(((t_local_total(d(i),j)+dt-beta(d(i),1))/(alpha(d(i),1)))+B1(d(i),k)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),6)=t_local_total(d(i),j)+2*dt;%+2e-6*randn+exprnd(4e-6);
                C(d(i),7)=(((t_local_total(d(i),j)+2*dt-beta(d(i),1))/(alpha(d(i),1)))+B1(d(i),k)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),8)=t_local_total(d(i),j)+3*dt;%+2e-6*randn+exprnd(4e-6);
                C(d(i),9)=(((t_local_total(d(i),j)+3*dt-beta(d(i),1))/(alpha(d(i),1)))+B1(d(i),k)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),10)=t_local_total(d(i),j)+4*dt;%+2e-6*randn+exprnd(4e-6);
                C(d(i),11)=(((t_local_total(d(i),j)+4*dt-beta(d(i),1))/(alpha(d(i),1)))+B1(d(i),k)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),12)=l(d(i),1);
                Neb_list{k}=C;
            end
        end
...
skewFinal1=mean(skew);



num_drones = 100; % 无人机数量
space_size = 3000; % 空间大小（长、宽、高均为1km）
min_speed =50; % 最小速度（m/s）
max_speed =60; % 最大速度（m/s）
comm_range = 1500; % 通信范围（m）
time_step = 0.1; % 时间步长（s）
speedChange=2;%无人机每秒速度的该变量（正态分布）
simulation_time = 500; % 模拟时间（s）
    ...
    for i=1:num_drones
        ...
        for k=1:num_drones
            if A(d(i),k)==1
                ...
                C(d(i),3)=(((t_local_total(d(i),j)-beta(d(i),1))/(alpha(d(i),1)))+B1(d(i),k)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),4)=t_local_total(d(i),j)+dt;%+2e-6*randn+exprnd(4e-6);
                C(d(i),5)=(((t_local_total(d(i),j)+dt-beta(d(i),1))/(alpha(d(i),1)))+(B1(d(i),k)+1)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),6)=t_local_total(d(i),j)+2*dt;%+2e-6*randn+exprnd(4e-6);
                C(d(i),7)=(((t_local_total(d(i),j)+2*dt-beta(d(i),1))/(alpha(d(i),1)))+(B1(d(i),k)+2)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),8)=t_local_total(d(i),j)+3*dt;%+2e-6*randn+exprnd(4e-6);
                C(d(i),9)=(((t_local_total(d(i),j)+3*dt-beta(d(i),1))/(alpha(d(i),1)))+(B1(d(i),k)+3)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),10)=t_local_total(d(i),j)+4*dt;%+2e-6*randn+exprnd(4e-6);
                C(d(i),11)=(((t_local_total(d(i),j)+4*dt-beta(d(i),1))/(alpha(d(i),1)))+(B1(d(i),k)+4)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),12)=l(d(i),1);
                Neb_list{k}=C;
            end
        end
    ...
    for i=1:num_drones
        ...
        for k=1:num_drones
            if A(d(i),k)==1
                ...
                C(d(i),3)=(((t_local_total(d(i),j)-beta(d(i),1))/(alpha(d(i),1)))+B1(d(i),k)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),4)=t_local_total(d(i),j)+dt;%+2e-6*randn+exprnd(4e-6);
                C(d(i),5)=(((t_local_total(d(i),j)+dt-beta(d(i),1))/(alpha(d(i),1)))+(B1(d(i),k)+2)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),6)=t_local_total(d(i),j)+2*dt;%+2e-6*randn+exprnd(4e-6);
                C(d(i),7)=(((t_local_total(d(i),j)+2*dt-beta(d(i),1))/(alpha(d(i),1)))+(B1(d(i),k)+4)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),8)=t_local_total(d(i),j)+3*dt;%+2e-6*randn+exprnd(4e-6);
                C(d(i),9)=(((t_local_total(d(i),j)+3*dt-beta(d(i),1))/(alpha(d(i),1)))+(B1(d(i),k)+6)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),10)=t_local_total(d(i),j)+4*dt;%+2e-6*randn+exprnd(4e-6);
                C(d(i),11)=(((t_local_total(d(i),j)+4*dt-beta(d(i),1))/(alpha(d(i),1)))+(B1(d(i),k)+8)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                C(d(i),12)=l(d(i),1);
                Neb_list{k}=C;
            end
        end
...
skewFinal3=mean(skew);

plot(5:simulation_k,skewFinal1(5:end),'-*','LineWidth',0.75);
hold on;
plot(6:simulation_k,skewFinal2(6:end),'-o','LineWidth',0.75);
hold on;
plot(6:simulation_k,skewFinal3(6:end),'-^','LineWidth',0.75);
grid on;
xlabel('同步轮次');
ylabel('全网最大频率差');
