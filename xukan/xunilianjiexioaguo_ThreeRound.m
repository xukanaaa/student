...
space_size = 6000; % 空间大小（长、宽、高均为1km）
...
round=5;%蒙特卡洛仿真轮数
skew1=zeros(round,simulation_k);%未平均之前的最大频偏差值矩阵(改进后)
skew2=zeros(round,simulation_k);%未平均之前的最大频偏差值矩阵(改进前)
skew3=zeros(round,simulation_k);%未平均之前的最大频偏差值矩阵(GTSP)

skewFinal_1=zeros(1,simulation_k);%平均后的每一轮频偏差值。(改进后)
skewFinal_2=zeros(1,simulation_k);%平均后的每一轮频偏差值。（改进前）
skewFinal_3=zeros(1,simulation_k);%平均后的每一轮频偏差值。（GTSP）

phySkew=80e-6;%物理时钟频偏最大值
phyOffset=1;%物理时钟相位偏移最大值
used_num=0;%每轮同步使用的数据量
M=zeros(num_drones+1,2);%新方法的返回值矩阵
...
for r=1:5
    ...
    %前n行表示首次接收到某节点的消息数据
    %后n行表示最新接收到某节点的消息数据
    Neb_list_new=cell(num_drones,1);
    for i=1:num_drones
        Neb_list_new{i}=zeros(num_drones*3,broad*2+2+1+num_drones);
    end
    ...
    for j=1:simulation_k
        for i=1:num_drones
            ...
            %第一轮不更新，从第二轮开始
            if j>=2
                M=update_xukan3_withTnoise_threeRound_l(Neb_list_new{d(i)},j,num_drones,l(d(i),1));%更新逻辑时钟频偏参数
            ...
            D1=Neb_list_new{d(i)};%广播节点的自身的消息矩阵
            %检查d(i)节点发送时与其有连接的其余节点，更新连接节点的邻居信息列表
            for k=1:num_drones
                if A(d(i),k)==1
                    C=Neb_list_new{k};%邻居的消息矩阵
    ...
    %旧方法
    l=ones(num_drones,1);
    Neb_list_old=cell(num_drones,1);
    for i=1:num_drones
        Neb_list_old{i}=zeros(num_drones*2,broad*2+2);
    end
    ...
    for j=1:simulation_k
        for i=1:num_drones
            ...
            %第一轮不更新，从第二轮开始
            if j>=2
                l(d(i),1)=update_xukan2_withTnoise_l(Neb_list_old{d(i)},j,num_drones,l(d(i),1));%更新逻辑时钟频偏参数
            ...
            for k=1:num_drones
                if A(d(i),k)==1
                    ...
                    if C(d(i),1)==0
                        ...
                        for k1=1:5
                            C(d(i),2*k1)=t_local_total(d(i),j)+dt*(k1-1)+2e-6*randn+2e-8*randn;
                            %C(d(i),2*k1)=t_local_total(d(i),j)+dt*(k1-1);
                        end
                        ...
                        for k2=1:5
                            C(d(i),2*k2+1)=(((t_local_total(d(i),j)+(k2-1)*dt-...
                                beta(d(i),1))/(alpha(d(i),1)))+B1(d(i),k)/3e8)*alpha(k,1)+beta(k,1)+2e-8*randn;
                            %C(d(i),2*k2+1)=(((t_local_total(d(i),j)+(k2-1)*dt-...
                            %   beta(d(i),1))/(alpha(d(i),1))))*alpha(k,1)+beta(k,1);
                        end
                        
                        C(d(i),12)=l(d(i),1);
                        Neb_list_old{k}=C;
                    end
                    %非首次最新接受到的消息（不管是那一轮），存放在第n+1到2n行，用于计算的数据是首次数据和最新数据
                    %注释掉的部分表示用于计算的数据是连续相邻两轮的数据点。
                    if C(d(i),1)~=0 %&& C(d(i)+num_drones,1)==0
                        C(d(i)+num_drones,1)=j;
                        for k1=1:5
                            C(d(i)+num_drones,2*k1)=t_local_total(d(i),j)+dt*(k1-1)+2e-6*randn+2e-8*randn;
                            %C(d(i)+num_drones,2*k1)=t_local_total(d(i),j)+dt*(k1-1);
                        end
                    ...
                    %                 if C(d(i),1)~=0 && C(d(i)+num_drones,1)~=0
                    %                      for k1=1:12
                    %                          C(d(i),k1)=C(d(i)+num_drones,k1);
                    %                      end
                    
                    %                     C(d(i)+num_drones,1)=j;
                    %                      for k1=1:5
                    %                         C(d(i)+num_drones,2*k1)=t_local_total(d(i),j)+dt*(k1-1)+2e-7*randn;
                    %                      end
                    %
                    %                      for k2=1:5
                    %                         C(d(i)+num_drones,2*k2+1)=(((t_local_total(d(i),j)+(k2-1)*dt-...
                    %                             beta(d(i),1))/(alpha(d(i),1)))+B1(d(i),k)/3e8)*alpha(k,1)+beta(k,1)+2e-7*randn;
                    %                      end
...
%                     C(d(i),7)=C(d(i),3);
%                     C(d(i),1)=j;
%                     C(d(i),2)=t_local_total(d(i),j)+8e-6*randn+2e-8*randn;
...
%五次仿真求平均值
skewFinal_1=mean(skew1);
skewFinal_2=mean(skew2);
% skewFinal_3=mean(skew3);



format long;
plot(1:simulation_k,skewFinal_1(1:end),'-*','LineWidth',0.75);
hold on;
plot(1:simulation_k,skewFinal_2(1:end),'-<','LineWidth',1);
hold on;
% plot(5:simulation_k,skewFinal_3(5:end),'->','LineWidth',1);
