...
space_size = 5000; % 空间大小（长、宽、高均为1km）
min_speed =80; % 最小速度（m/s）
max_speed =100; % 最大速度（m/s）
comm_range = 2000; % 通信范围（m）
    ...
    for i=1:num_drones
        ...
        for k=1:num_drones
            if A(d(i),k)==1
                ...
                C(d(i),3)=(((t_local_total(d(i),j)-beta(d(i),1))/(alpha(d(i),1)))+B(d(i),k)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
...
format long;
figure;
plot(12:simulation_k,skewFinal1(12:end),'-*','LineWidth',1);
hold on;
plot(9:simulation_k,skewFinal2(9:end),'-o','LineWidth',1);
