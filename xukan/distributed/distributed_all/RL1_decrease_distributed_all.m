...
%对同步实现后的网络进行优化，精度达标则减少同步邻居，平衡计算复杂度和同步精度
...
complexity=zeros(num_drones,simulation_k);%表征计算复杂度
complexity_results=zeros(round,simulation_k);
...
for r=1:2
    ...
    %这里写死了20轮能够收敛，采用虚拟链接加快速度
    %todo:需要网络自身判断状态，来决定采用的同步策略
    Neb_count1=zeros(num_drones,1);
    for j=1:20
        for i=1:num_drones
    ...
    %每次更新所用的邻居数，用于平衡计算复杂度和同步精度
    Neb_count=zeros(num_drones,1);
    %20轮之后不需要虚拟链接，同时根据方差逐步调整用于同步的邻居个数
    for j=21:simulation_k
        for i=1:num_drones
            [~,d]=sort(t_global_total(:,j));
            
            %当网络失去同步，需要把同步矩阵置零，重新开始同步
            %todo:这里是写死了。后续也需要通过判断网络状态来自己转换
            if j==101
                for i1=1:num_drones
                    Neb_list_fre{i1}=zeros(num_drones*2,broad*2+2+1+num_drones);
                end
                for i1=1:num_drones
                    Neb_list_pha{i1}=zeros(num_drones,1+broad*2+2+1+num_drones);
                end
            end
            
            %第一轮不更新，从第二轮开始
            if j~=101
                M1=distributed_all_vary_l(Neb_list_fre{d(i)},j,num_drones,l(d(i),1),Neb_count(d(i),1),d(i),l,h);%更新逻辑时钟频偏参数
                %这个返回的M矩阵其实包含了在这一轮更新中收到真实邻居的αij*l的值，后面广播可以放到该节点携带的虚拟信息列表中。
                l(d(i),1)=M1(1,1);%更新逻辑时钟频偏参数
                Neb_count(d(i),1)=M1(1,2);%取出更新使用的数据量，决定下一轮是否广播求助信息，让接收到该信息的邻居节点广播二跳信息。
                %更新完之后，把右侧n列关于二跳节点的信息清零(频率)
                E=Neb_list_fre{d(i)};
                E(:, 14:end)=0;
                Neb_list_fre{d(i)}=E;
                
                N1=distributed_all_vary_h(Neb_list_pha{d(i)},j,num_drones,Neb_count(d(i),1),d(i),l,h);%更新逻辑时钟相偏参数
                h(d(i),1)=N1;%更新逻辑时钟相偏参数
                %更新完之后，把右侧n列关于二跳节点的信息清零(相位)
                E=Neb_list_pha{d(i)};
                E(:, 15:end)=0;
                Neb_list_pha{d(i)}=E;
            ...
            if j>100
                A=I{ceil((t_local_total(d(i),j)-beta(d(i),1))/(alpha_vary(d(i),1)*time_step))};%发送消息时刻的连接矩阵
                B1=D{ceil((t_local_total(d(i),j)-beta(d(i),1))/(alpha_vary(d(i),1)*time_step))};%发送消息时刻的距离矩阵
                B2=D{ceil((t_local_total(d(i),j)-beta(d(i),1))/(alpha_vary(d(i),1)*time_step))+1};
            end
                ...
                if A(d(i),k)==1
                    ...
                    if F(d(i),1)==0
                        ...
                        if j>100
                            for k2=1:5
                                F(d(i),2*k2+1)=(((t_local_total(d(i),j)+(k2-1)*dt-...
                                    beta(d(i),1))/(alpha_vary(d(i),1)))+B1(d(i),k)/3e8)*alpha_vary(k,1)+beta(k,1)+5e-9*randn;
                                P(d(i),2*k2+1)=F(d(i),2*k2+1);
                            end
                        end
                        ...
                        if j>100
                            T4=(((T3-beta(k,1))/(alpha_vary(k,1)))+B1(d(i),k)/3e8)*alpha_vary(d(i),1)+beta(d(i),1)+5e-9*randn;
                        end
                        Td=(T2+T4-T3-T1)/2;
                        P(d(i),14)=Td;
    ...
    %复杂度矩阵案列求和
    complexity_results(r,:)=sum(complexity);
...
complexity_result=mean(complexity_results);
...
% hold on;
%semilogy(1:simulation_k,clockFinal(1:end),'-r*','LineWidth',1);
%plot(1:simulation_k,complexity_result(1:end),'-*','LineWidth',1);
plot(5:5:simulation_k,complexity_result(5:5:end),'-*','LineWidth',1);
xlabel('同步轮数');  % 添加x轴标签
ylabel('该轮次下全网用于同步的邻居数');  
ylim([0, 5000]);
% 添加y轴标签
grid on;
hold on;
