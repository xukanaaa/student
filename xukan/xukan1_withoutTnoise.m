...
for j=1:simulation_k
    for i=1:num_drones
        [c,d]=sort(t_global_total(:,j));
             ...
             %第一轮不更新，从第二轮开始
        if j>=2
            l(d(i),1)=update_xukan1_withoutTnoise_l(Neb_list{d(i)},j,num_drones,l(d(i),1));%更新逻辑时钟频偏参数
        ...
        for k=1:num_drones
            if A(d(i),k)==1
                ...
                C(d(i),1)=j;
                for k1=1:5
                    C(d(i),2*k1)=t_local_total(d(i),j)+dt*(k1-1);%+2e-6*randn+exprnd(4e-6);
                end
                
                for k1=1:5
                    C(d(i),2*k1+1)=(((t_local_total(d(i),j)+dt*(k1-1)-beta(d(i),1))/(alpha(d(i),1)))+B1(d(i),k)/3e8)*alpha(k,1)+beta(k,1);%+2e-6*randn+exprnd(4e-6);
                end
