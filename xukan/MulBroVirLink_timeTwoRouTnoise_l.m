...
for n=(i+1):(2*i)
    ...
    if A(n+i,1)>=(j-1)
        ...
        if A(n-i,1) ~=0
            ...
            if abs(A(n+i,1)-A(n-i,1))<=10 %当两次消息间隔轮次过大时，不需要进行偏移补偿
                ...
                new_x=(A(n+i,3)+A(n+i,5)+A(n+i,7)+A(n+i,9)+A(n+i,11))/5;%最新接收消息的x轴平均值
                new_y=(A(n+i,2)+A(n+i,4)+A(n+i,6)+A(n+i,8)+A(n+i,10))/5;%最新接收消息的y轴平均值
                
                %d调整的式子的分子
                d_molecule=0;
                for k1=1:5
                    d_molecule=d_molecule+(A(n+i,2*k1)-old_y)*(new_x-old_x)*(A(n+i,2*k1+1)-...
                        new_x)-(A(n+i,2*k1+1)-old_x)*(A(n+i,2*k1+1)-new_x)*(new_y-old_y)...
                        -(A(n-i,2*k1+1)-old_x)*(A(n-i,2*k1+1)-old_x)*(new_y-old_y)-...
                        (old_y-A(n-i,2*k1))*(A(n-i,2*k1+1)-old_x)*(new_x-old_x);
                end
                
                %d调整式子的分母
                d_denominator=(A(n+i,3)-new_x)^2+(A(n+i,5)-new_x)^2+(A(n+i,7)-new_x)^2+...
                    (A(n+i,9)-new_x)^2+(A(n+i,11)-new_x)^2+...
                ...
                for k2=1:5
                    A(n+i,2*k2)=A(n+i,2*k2)+d;
                end
            end
            
            %对第二轮消息进行偏移d的处理，让其尽量与第一次数据在同一直线上
            if abs(A(n,1)-A(n-i,1))<=10 %当两次消息间隔轮次过大时，不需要进行偏移补偿
                old_x=(A(n-i,3)+A(n-i,5)+A(n-i,7)+A(n-i,9)+A(n-i,11))/5;%首次接收消息的x轴平均值
                old_y=(A(n-i,2)+A(n-i,4)+A(n-i,6)+A(n-i,8)+A(n-i,10))/5;%首次接收消息的y轴平均值
    ...
    elseif  A(n,1)>=j-1
        ...
        if A(n-i,1)~=0
            ...
            %利用第一次的消息进行线性回归求斜率
        Tj_mean_old=(A(n,2)+A(n,4)+A(n,6)+A(n,8)+A(n,10))/5;
        Ti_mean_old=(A(n,3)+A(n,5)+A(n,7)+A(n,9)+A(n,11))/5;
        molecule=(A(n,2)-Tj_mean_old)*(A(n,3)-Ti_mean_old)+(A(n,4)-Tj_mean_old)*(A(n,5)-Ti_mean_old)...
            +(A(n,6)-Tj_mean_old)*(A(n,7)-Ti_mean_old)+(A(n,8)-Tj_mean_old)*(A(n,9)-Ti_mean_old)+...
            (A(n,10)-Tj_mean_old)*(A(n,11)-Ti_mean_old);
        denominator=(A(n,3)-Ti_mean_old)^2+(A(n,5)-Ti_mean_old)^2+(A(n,7)-Ti_mean_old)^2+...
            (A(n,9)-Ti_mean_old)^2+(A(n,11)-Ti_mean_old)^2;
        
        deltal(n,1)=molecule*A(n,12)/denominator;%把首次接收到消息的节点的更新值加入deltal
        m=m+1;
        end
