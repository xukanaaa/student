...
for r=1:5
    ...
    for i=1:num_drones
        Neb_list_new{i}=zeros(num_drones*2,broad*2+2+1+num_drones);
    end
    ...
    for j=1:simulation_k
        for i=1:num_drones
            ...
            %第一轮不更新，从第二轮开始
            if j>=2
                M=MulBroVirLink_alphaTwoRouTnoise_l(Neb_list_new{d(i)},j,num_drones,l(d(i),1));%更新逻辑时钟频偏参数
...
format long;
%plot(5:simulation_k,skewFinal_1(5:end),'-*','LineWidth',0.75);
plot(3:simulation_k,skewFinal_2(3:end),'-<','LineWidth',1);
grid on;
hold on ;
xlabel('同步轮数');
ylabel('全网最大频率差');
