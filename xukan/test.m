...
round=5;%蒙特卡洛仿真轮数
skew1=zeros(round,simulation_k);%未平均之前的最大频偏差值矩阵
skew2=zeros(round,simulation_k);
skew3=zeros(round,simulation_k);
...
for j=1:simulation_k
    for i=1:num_drones
        [c,d]=sort(t_global_total(:,j));
             ...
             %第一轮不更新，从第二轮开始
        if j>=2
            l(d(i),1)=update_xukan2_withTnoise_l(Neb_list{d(i)},j,num_drones,l(d(i),1));%更新逻辑时钟频偏参数
        ...
        for k=1:num_drones
            if A(d(i),k)==1               
                ...
                if C(d(i),1)==0
                    C(d(i),1)=j;
                    for k1=1:5
                        C(d(i),2*k1)=t_local_total(d(i),j)+dt*(k1-1)+2e-7*randn;
                    end
...
for i=1:num_drones
    Neb_list1{i}=zeros(num_drones,broad*2+2);
end
...
for j=1:simulation_k
    for i=1:num_drones
        [c,d]=sort(t_global_total(:,j));
             ...
             %第一轮不更新，从第二轮开始
        if j>=2
            l(d(i),1)=update_xukan1_withTnoise_l(Neb_list1{d(i)},j,num_drones,l(d(i),1));%更新逻辑时钟频偏参数
            %h(d(i),1)=update_h(Neb_list{d(i)},j,num_drones,h(d(i),1));%更新逻辑时钟相偏参数
        end
...
%初始化每个节点本地的邻居信息列表(消息轮，当前邻居物理，当前自身物理，当前邻居逻辑，当前自身逻辑，之前邻居物理，之前自身物理，当前邻居l)
    Neb_list2=cell(num_drones,1);
    for i=1:num_drones
        Neb_list2{i}=zeros(num_drones,8);
    end
    ...
    for j=1:simulation_k
        for i=1:num_drones
            ...
            %第一轮不更新，从第二轮开始
            if j>=2
                l(d(i),1)=update_l(Neb_list2{d(i)},j,num_drones,l(d(i),1));%更新逻辑时钟频偏参数
                h(d(i),1)=update_h(Neb_list2{d(i)},j,num_drones,h(d(i),1));%更新逻辑时钟相偏参数
            end
...
format long;
plot(3:simulation_k,skewFinal_1(3:end),'-*','LineWidth',1);
hold on;
 plot(5:simulation_k,skewFinal_2(5:end),'-<','LineWidth',1);
 hold on;
