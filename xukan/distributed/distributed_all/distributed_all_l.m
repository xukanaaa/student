function l=distributed_all_l(A,j,i,L)
% A:节点自身维护的消息矩阵
% j:当前同步轮数
% i:无人机总数
% L:自身节点的频偏补偿值
% l:返回值矩阵。第一行分别是更新用的l,收到的邻居数量
%后面的每一行，第一列表示同步轮数，第二列表示用于更新的值
...
for n=(i+1):(2*i)
    ...
    if A(n,1)>=(j-1)
        ...
        if A(n,1)-A(n-i,1)<=50 %当两次消息间隔轮次过大时，不需要进行偏移补偿
       ...
       for k5=14:13+i
           if (A(n,k5)~=0)&&(~(A(k5-14+1,1)>=j-1||A(k5-14+1+i,1)>=j-1))
    ...
    %第一次接收到该邻居消息且是最新消息
     if A(n,1)==0 && A(n-i,1)>=j-1
       ...
       %遍历该有效邻居信息的二跳矩阵（后n列），计算结果加入deltal中，递增计数器m
       for k5=14:13+i
           if (A(n-i,k5)~=0)&&(~(A(k5-14+1,1)>=j-1||A(k5-14+1+i,1)>=j-1))
