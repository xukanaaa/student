...
if A(k+i,1)>=(j-1)&& A(k+i,14)==0
    %对第二轮消息进行偏移d的处理，让其尽量与第一次数据在同一直线上
    if A(k+i,1)-A(k,1)<20 %当两次消息间隔轮次过大时，不需要进行偏移补偿
        old_x=(A(k,3)+A(k,5)+A(k,7)+A(k,9)+A(k,11))/5;%首次接收消息的x轴平均值
        old_y=(A(k,2)+A(k,4)+A(k,6)+A(k,8)+A(k,10))/5;%首次接收消息的y轴平均值
        ...
        for k1=1:5
            d_molecule=d_molecule+(A(k+i,2*k1)-old_y)*(new_x-old_x)*(A(k+i,2*k1+1)-...
                ...
                -(A(k,2*k1+1)-old_x)*(A(k,2*k1+1)-old_x)*(new_y-old_y)-...
                (old_y-A(k,2*k1))*(A(k,2*k1+1)-old_x)*(new_x-old_x);
        end
        
        %d调整式子的分母
        d_denominator=(A(k+i,3)-new_x)^2+(A(k+i,5)-new_x)^2+(A(k+i,7)-new_x)^2+...
            (A(k+i,9)-new_x)^2+(A(k+i,11)-new_x)^2+...
            (A(k,3)-old_x)^2+(A(k,5)-old_x)^2+(A(k,7)-old_x)^2+...
            (A(k,9)-old_x)^2+(A(k,11)-old_x)^2;
    ...
    %首轮消息就是最新消息，这时第二轮消息是多余的最新的二跳，不管，只进行首轮回归
else
    %利用第一次的消息进行线性回归求斜率
