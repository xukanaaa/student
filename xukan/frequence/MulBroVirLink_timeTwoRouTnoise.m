...
for r=1:5
    ...
    %flag位，只占该列第一行，下面都是0,多跳信息是否覆盖位（测量值为0，多跳信息由一轮数据估计为1,2轮为2,0覆盖12,2覆盖1等等）
    %前n行表示首次接收到某节点的消息数据
    %后n行表示最新接收到某节点的消息数据
        ...
        for i=1:num_drones
            ...
            for k=1:num_drones
                if A(d(i),k)==1
                    ...
                    %首次接收到的消息（不管是第几轮）存放在前n行，n表示无人机总数，第一轮不存放二跳信息
                    if C(d(i),1)==0
                        C(d(i),1)=j;
                        C1(d(i),1)=j;
                        ...
                        %添加规则是，第一轮是一跳信息
                        %如果第二轮不是最新消息or第二轮是最新消息也是二跳且估计数据集小于当前二跳数据集，则覆盖
                        %这样，同一轮可能受到同一个二跳节点信息，取其中较准确的值。最新的二跳信息不会覆盖最新的一跳信息
                        if D1(1,13)==1
                            %遍历邻居列表，找出上一轮同步的邻居
                            for k3=1:num_drones
                                if D2(k3,1)>=j-1
                                    ...
                                    [alphat,m1]=compensate_TwoRou(D1,k3,num_drones,j);%返回斜率和估计所用的轮数
                                    deltat_y=alphat*deltat;%用于对二跳邻居时钟补偿的时间差
                                    
                                    %k3对应的二跳邻居信息是否可以加入二轮同步矩阵
                                    %（第二轮不是最新)或者（第二轮是最新消息也是二跳且估计数据集小于当前二跳估计数据集），则覆盖
                                    if (C(k3+num_drones,1)<j-1)||(C(k3+num_drones,1)>=j-1&&C(k3+num_drones,14)>0&&C(k3+num_drones,14)<m1)
