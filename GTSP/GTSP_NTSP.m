...
x1=zeros(num_drones,simulation_k);
y1=zeros(num_drones,simulation_k);
z1=zeros(num_drones,simulation_k);
deltamax1=zeros(simulation_k,1);%每轮迭代后全网最大钟差
t1=zeros(simulation_k,1);%每轮迭代后用于测评的真实时间，此处选为最后一个发送消息的节点每次发送完的时间
...
%初始化逻辑时钟频偏
l1=ones(num_drones,1);
%初始化逻辑时钟相偏
h1=zeros(num_drones,1);
% 初始化无人机通信邻接矩阵
...
for i=1:num_drones
    Neb_list1{i}=zeros(num_drones,8);
end
    ...
    for i=1:num_drones
        ...
        for k=1:num_drones
            if A(d(i),k)==1
                ...
                C(d(i),7)=C(d(i),3);
                C(d(i),1)=j;
                C(d(i),2)=t_local_total(d(i),j);
                C(d(i),3)=(((t_local_total(d(i),j)-beta(d(i),1))/(alpha(d(i),1)))+B(d(i),k)/3e8)*alpha(k,1)+beta(k,1);
...
x2=zeros(simulation_k,1);
y2=zeros(simulation_k,1);
for c=1:simulation_k
    x2(c,1)=max(x(:,c))-min(x(:,c));
end
for d=1:simulation_k
    y2(d,1)=max(y(:,d))-min(y(:,d));
end
...
for j=1:simulation_k
    for i=1:num_drones
        [c1,d1]=sort(t_global_total(:,j));
        
        %第一轮不更新，从第二轮开始
        if j>=2
            l1(d1(i),1)=update1_l(Neb_list1{d1(i)},j,num_drones,l1(d1(i),1));%更新逻辑时钟频偏参数
            h1(d1(i),1)=update1_h(Neb_list1{d1(i)},j,num_drones,h1(d1(i),1));%更新逻辑时钟相偏参数
        ...
        A1=I{ceil((t_local_total(d1(i),j)-beta(d1(i),1))/(alpha(d1(i),1)*time_step))};%发送消息时刻的连接矩阵
        B1=D{ceil((t_local_total(d1(i),j)-beta(d1(i),1))/(alpha(d1(i),1)*time_step))};%发送消息时刻的距离矩阵
        ...
        for k=1:num_drones
            if A1(d1(i),k)==1
                ...
                C1(d1(i),6)=C1(d1(i),2);
                C1(d1(i),7)=C1(d1(i),3);
                C1(d1(i),2)=t_local_total(d1(i),j);
                C1(d1(i),3)=(((t_local_total(d1(i),j)-beta(d1(i),1))/(alpha(d1(i),1)))+B1(d1(i),k)/3e8)*alpha(k,1)+beta(k,1);
                C1(d1(i),4)=l1(d1(i),1)*C1(d1(i),2)+h1(d1(i),1);
                C1(d1(i),5)=l1(k,1)*C1(d1(i),3)+h1(k,1);
                C1(d1(i),8)=l1(d1(i),1);
                ...
                if j>2
                    if ((l_total(d1(i),j-1)-l_total(d1(i),j-2))<=thre)
                        C1(d1(i),1)=j;
                    end
                    if ((l_total(d1(i),j-1)-l_total(d1(i),j-2))>thre)
                        C1(d1(i),1)=-j;
...
for i=1:simulation_k
    t1(i,1)=(t_local_total(num_drones,i)-beta(num_drones))/alpha(num_drones);
end
for k=1:simulation_k
    deltamax1(k,1)=max(x1(:,k)*t1(k,1)+y1(:,k))-min(x1(:,k)*t1(k,1)+y1(:,k));
end

x3=zeros(simulation_k,1);
y3=zeros(simulation_k,1);
for c=1:simulation_k
    x3(c,1)=max(x1(:,c))-min(x1(:,c));
end
for d=1:simulation_k
    y3(d,1)=max(y1(:,d))-min(y1(:,d));
end

com=zeros(simulation_k,2);
com(:,1)=x2;
com(:,2)=x3;
format long;
disp(com)