 function deltaTd=compensate_pha(C,P,k,n)
%对相位消息的距离偏移进行补偿
...
d_molecule=0;
for k1=1:5
    d_molecule=d_molecule+(C(n+k,2*k1)-old_y)*(new_x-old_x)*(C(n+k,2*k1+1)-new_x)-(C(n+k,2*k1+1)-old_x)*(C(n+k,2*k1+1)-new_x)*(new_y-old_y)...
