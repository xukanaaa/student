space_sizet=[2000,4000,6000];
comm_ranget=[1000,1500,2000];
skewFinal=zeros(3,simulation_k);%平均后的每一轮频偏差值。

for s=1:3
    
num_drones = 100; % 无人机数量
space_size =space_sizet(s); % 空间大小（长、宽、高均为1km）
min_speed =80; % 最小速度（m/s）
max_speed =100; % 最大速度（m/s）
comm_range = comm_ranget(s); % 通信范围（m）
...
skewFinal(s,:)=mean(skew);
end

format long;
plot(3:simulation_k,skewFinal(1,3:end),'-*','LineWidth',0.75);
hold on;
plot(5:simulation_k,skewFinal(2,5:end),'-*','LineWidth',0.75);
hold on;
plot(7:simulation_k,skewFinal(3,7:end),'-*','LineWidth',0.75);
xlabel('同步轮次');
ylabel('最大频率差');
