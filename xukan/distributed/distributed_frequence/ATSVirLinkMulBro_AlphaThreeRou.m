...
for i=1:num_drones
    Neb_list{i}=zeros(num_drones*2,broad*2+2+1+2);
end
...
for j=1:simulation_k
    for i=1:num_drones
        ...
        %检查当前广播节点的邻居数量，如果小于某个数，则向邻居求救，其邻居就会在下次广播中附加二跳节点信息。
        countNeb = 0;
        for k=1:num_drones
            if(A(d(i),k)==1)
                countNeb=countNeb+1;
        ...
        for k=1:num_drones
            if A(d(i),k)==1
                ...
                %每个收到广播的节点在完成数据填充后，开始更新自己的逻辑时钟频率。
                [l(k,1),ne]=ATSVirLinkMulBro_AlphaThreeRou_l(Neb_list{k},num_drones,l(k,1),d(i));
                %跟新完后，二跳邻居置零
                C1=Neb_list{k};
                C1(:,14)=0;
                C1(1,15)=ne;
