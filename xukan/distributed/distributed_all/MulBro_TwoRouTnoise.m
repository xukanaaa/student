...
for r=1:5
    ...
    for j=1:simulation_k
        for i=1:num_drones
            ...
            %第一轮不更新，从第二轮开始
            if j>=2
                l(d(i),1)=MulBro_TwoRouTnoise_l(Neb_list_fre{d(i)},j,num_drones,l(d(i),1));%更新逻辑时钟频偏参数
            ...
            for k=1:num_drones
                if A(d(i),k)==1
                    ...
                    if C(d(i),1)==0
                        ...
                        T2=C(d(i),3);
                        T3=T2+20e-3;
                        T4=(((T3-beta(k,1))/(alpha(k,1)))+B1(d(i),k)/3e8)*alpha(d(i),1)+beta(d(i),1)+2e-9*randn;
                        Td=(T2+T4-T3-T1)/2;
                        P(d(i),14)=Td;
                        C(d(i),12)=l(d(i),1);
                        Neb_list_fre{k}=C;
                    ...
                    if C(d(i),1)~=0
                        ...
                        P(d(i),1)=j;
                        for k1=1:5
                            C(d(i)+num_drones,2*k1)=t_local_total(d(i),j)+dt*(k1-1)+2e-9*randn;
                        end
                        ...
                        for k1=1:5
                            ...
                            P(d(i),2*k1+1)=C(d(i)+num_drones,2*k1+1);
                        end
                        P(d(i),12)=l(d(i),1);
                        P(d(i),13)=h(d(i),1);
