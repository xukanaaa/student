%强化学习自适应周期同步过程中的逻辑时钟频率调整
function [l,F]=distributed_all_l_RL(A,i,L)
% A:节点自身维护的消息矩阵
% i:无人机总数
% L:自身节点的频偏补偿值
% l:更新用的l
% F:改变后的A矩阵的值，把更新后的数据进行处理
...
for n=(i+1):(2*i)
    ...
    if A(n,1)==1
        ...
        if A(n,2)-A(n-i,2) <=20 %当两次消息间隔轮次过大时，不需要进行偏移补偿
       ...
       m=m+1;
       temp_A(n,1)=0;%同步完后把同步数据标记为旧数据
    end
    
    %第一次接收到该邻居消息且是最新消息
     if A(n,1)==0 && A(n-i,1)==1
        %利用第一次的消息进行线性回归求斜率
       ...
       Ti_mean_old=(A(n-i,3)+A(n-i,5)+A(n-i,7)+A(n-i,9)+A(n-i,11))/5;
        ...
        m=m+1;
        temp_A(n-i,1)=0;%同步完后把同步数据标记为旧数据                      
     end
end
l=(sum(deltal)+L)/(m+1);
F=temp_A;
