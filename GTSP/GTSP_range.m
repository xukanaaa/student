num_dronest = [100,150,200]; % 无人机数量
space_sizet = [4000,6000,8000]; % 空间大小（长、宽、高均为1km）
min_speed =90; % 最小速度（m/s）
max_speed =100; % 最大速度（m/s）
comm_ranget = [2000,3000,4000]; % 通信范围（m）


for s=1:3
    num_drones=num_dronest(s);
    space_size=space_sizet(s);
    comm_range=comm_ranget(s);
...
format long;
plot(15:simulation_k,skewFinal(1,15:end),'-*','LineWidth',1);
hold on;
plot(15:simulation_k,skewFinal(2,15:end),'-o','LineWidth',1);
hold on;
plot(15:simulation_k,skewFinal(3,15:end),'-s','LineWidth',1);
xlabel('同步轮次');
ylabel('最大频率偏差')
%disp(x1)